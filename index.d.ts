...
interface ParticlesNode extends BaseNode {
  ...
  image: string | null;
  spritesheet:
    | [rows: number, columns: number, fps: number, loop: boolean]
    | null;
...
type ImageInit = NodeInitBase &
  Partial<
    ...
    >
  >;
type MeshInit = NodeInitBase &
  Partial<Pick<MeshNode, "castShadow" | "receiveShadow">>;
type SkinnedMeshInit = NodeInitBase &
  Partial<Pick<SkinnedMeshNode, "castShadow" | "receiveShadow">>;
type RigidBodyInit = NodeInitBase &
  Partial<
    ...
    >
  >;
type ColliderInit = NodeInitBase &
  Partial<Pick<ColliderNode, "type" | "radius" | "convex" | "trigger">>;
type ControllerInit = NodeInitBase &
  Partial<
    Pick<
      ControllerNode,
      "radius" | "height" | "layer" | "tag" | "onContactStart" | "onContactEnd"
    >
  >;
type LODInit = NodeInitBase & Partial<Pick<LODNode, "scaleAware">>;
type AvatarInit = NodeInitBase &
  Partial<Pick<AvatarNode, "src" | "emote" | "visible">>;
...
type ActionInit = NodeInitBase &
  Partial<
    Pick<
      ActionNode,
      "label" | "distance" | "duration" | "onStart" | "onTrigger" | "onCancel"
    >
  >;
...
interface AppAPI extends BaseNode {
  ...
  create<TName extends NodeName>(
    name: TName,
    props?: NodeNameToInit[TName]
  ): NodeNameToType[TName];
...
export {};
/*
 Ambient typings for Hyperfy App Server runtime. Consumers can add
 "types": ["@drama.haus/app-server"] in their tsconfig to get editor hints
 for world/app APIs when writing scripts. These are declarations-only.
*/
